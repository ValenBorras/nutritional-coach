...
export default function NutritionChatbot({ 
  ...
  const generateIntelligentTitle = async (userMessage: string, assistantResponse: string): Promise<string> => {
    try {
    ...
    try {
      ...
      if (isFirstUserMessage && !currentChatId) {
        try {
          ...
          const intelligentTitle = await generateIntelligentTitle(currentInput, data.message)
          ...
          if (createResponse.ok) {
            ...
            console.log(`✅ Conversación creada y guardada: "${intelligentTitle}"`)
          }
        } catch (error) {
          console.error('Error creating conversation after successful exchange:', error)
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {loadingChats ? (
              ...
              <div className="text-center py-8 text-charcoal/60 text-sm">
                No hay conversaciones guardadas.<br/>
                <span className="text-xs text-charcoal/50">Las conversaciones se guardan después del primer intercambio completo</span>
              </div>
            ) : (
                    ...
                    <div className="text-sm leading-relaxed [&_li_p]:inline [&_li_p]:m-0 [&_li]:flex [&_li]:items-start [&_li]:gap-1">
                      <ReactMarkdown 
                        components={{
                          ...
                          li: ({ children, ...props }) => (
                            <li className="text-sm leading-relaxed flex items-start gap-2 mb-1" {...props}>
