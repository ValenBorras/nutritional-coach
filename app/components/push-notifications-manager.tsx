...
export default function PushNotificationsManager({ className }: PushNotificationsManagerProps) {
  const {
    permission,
    isSupported,
    isSubscribed,
  ...
  const handleToggleNotifications = async () => {
    if (isSubscribed) {
      ...
      if (success) {
        toast.success('¡Notificaciones activadas! Recibirás recordatorios útiles')
      } else {
          ...
          <CardDescription>
            {isSafari ? (
              <div className="space-y-2">
                <p>Estamos configurando las notificaciones para Safari. Esto puede tomar unos segundos...</p>
                {!isStandalone && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-800">
                      <strong>💡 Para habilitar notificaciones en Safari:</strong>
                    </p>
                    <ol className="text-sm text-blue-700 mt-1 ml-4 list-decimal">
                      <li>Asegúrate de tener iOS 16.4+ o macOS Safari 16+</li>
                      <li>Instala la app usando "Agregar a pantalla de inicio"</li>
                      <li>Abre la app desde la pantalla de inicio (no desde Safari)</li>
                      <li>Las notificaciones estarán disponibles en unos segundos</li>
                    </ol>
                  </div>
                )}
                {isStandalone && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm text-green-800">
                      ✅ App PWA detectada. Las notificaciones deberían activarse automáticamente...
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex items-center space-x-2">
                  ...
                  <span>Service Worker: {'serviceWorker' in navigator ? 'Sí' : 'No'}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full ${
                    'PushManager' in window ? 'bg-green-500' : 'bg-red-500'
                  }`} />
                  <span>Push Manager: {'PushManager' in window ? 'Sí' : 'No'}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full ${
                    'Notification' in window ? 'bg-green-500' : 'bg-red-500'
                  }`} />
                  <span>Notifications: {'Notification' in window ? 'Sí' : 'No'}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <div className={`w-2 h-2 rounded-full ${
  ...
  return (
    <Card className={`${className} border-soft-rose/20 bg-gradient-to-br from-warm-sand to-warm-sand/80`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <StatusIcon className={`h-5 w-5 ${statusInfo.color}`} />
            <CardTitle className="text-lg text-charcoal">Notificaciones Push</CardTitle>
          </div>
          <Badge 
            variant="secondary" 
        ...
        {permission === 'denied' && (
          <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-medium text-amber-800 mb-1">Permisos bloqueados</h4>
                <p className="text-sm text-amber-700 mb-2">
                  Para recibir notificaciones, necesitas habilitar los permisos en tu navegador:
                </p>
                <ol className="text-xs text-amber-700 space-y-1 ml-4 list-decimal">
                  ...
                  <li>Busca la opción "Notificaciones" y selecciona "Permitir"</li>
        ...
        {permission !== 'denied' && (
          <div className="flex items-center justify-between p-3 bg-white/50 rounded-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <Label htmlFor="notifications-toggle" className="text-sm font-medium text-charcoal">
        ...
        <div className="grid grid-cols-2 gap-3 pt-2">
          ...
          <div className="flex items-center space-x-2 text-xs">
            <div className={`w-2 h-2 rounded-full ${
              ...
              permission === 'denied' ? 'bg-red-500' : 'bg-orange-500'
            }`} />
            <span className="text-charcoal/60">
