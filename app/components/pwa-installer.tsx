...
async function registerServiceWorkerRobust() {
  ...
  try {
    ...
    if (!swFileCheck.ok) {
      console.error('❌ Service worker file not accessible:', swFileCheck.status)
      return null
    }

    // Check for existing registrations first
    const existingRegistrations = await navigator.serviceWorker.getRegistrations()
    console.log(`🔍 Found ${existingRegistrations.length} existing SW registrations`)
    ...
    if (existingRegistrations.length > 0) {
      try {
        ...
        console.log('⚠️ Existing SW not ready within timeout, will re-register')
      }
    }
    ...
    try {
      ...
      console.log('⚠️ SW ready timeout, but registration exists. This might still work.')
      return registration
    }
    ...
    if (isSafari) {
      console.log('🍎 Safari detected, trying alternative registration approach...')
      ...
      try {
        ...
        const retryRegistration = await navigator.serviceWorker.register('/sw.js')
        ...
        try {
          ...
          const finalRegistration = await navigator.serviceWorker.register('/sw.js', { scope: '/' })
      ...
      <div className="bg-[#F5EBDD] dark:bg-[#4A4A4A] border border-[#F7CAC9] dark:border-[#A8CBB7] rounded-lg shadow-lg p-4">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-sm text-[#4A4A4A] dark:text-[#F5EBDD]">Instalar NutriGuide</h3>
          </div>
          <Button
            variant="ghost"
            size="sm"
        ...
        <p className="text-sm text-[#4A4A4A] dark:text-[#F5EBDD] mb-3">
          Instala NutriGuide en tu dispositivo para acceso rápido y funcionalidad offline.
        </p>
