...
export default function PWARedirect() {
  ...
  useEffect(() => {
    ...
    if (isPWA) {
      console.log(`🔄 PWA access detected (${platform}, ${source}), checking authentication...`)
      setShowLoading(true)
        ...
        if (hasStoredSession) {
          ...
          const timeoutId = setTimeout(() => {
            if (!user && !authUser && !hasRedirected) {
              console.log('🔐 No valid session found after waiting, redirecting to login')
